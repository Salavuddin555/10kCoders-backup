...
export class Form4Server extends Component {
    constructor(){
        super();
        this.state={
            person:{
 ...
 this.setState({person:user,editIndex:i})
    ...
    deleteUser=(usr,id)=>{
        var number=id+1
        axios.delete("http://localhost:3004/users/"+number).then((res)=>
    ...
    updateUser=()=>{
        ...
        axios({
            method:"put",
            url:"http://localhost:3004/users"+number,
            data:this.state.person
   ...
   async componentDidMount(){
            var response=await axios("http://localhost:3004/users")
